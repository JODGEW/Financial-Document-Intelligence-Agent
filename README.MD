...
**Maturity, stated plainly: this is a Stage 2.5 filing-comparison prototype** — the Stage 2 governed Q&A loop (evidence, validation, risk routing, local human review) plus a first executable end-to-end filing comparison: a persisted workflow (create → detect → reopen → result) over durable filing identities, structured changes carrying two-sided chunk-resolvable evidence, and controlled labeled metrics reported honestly (precision / recall / change-type accuracy / evidence resolution = 1.0 over one synthetic Acme pair — a controlled detector check, **not** a production benchmark; nothing has been measured on real filings). The next target is a **Stage 3 controlled review workflow**. Stage 3 is not achieved.
...
7. Comparison-specific deterministic validation — **partially done, the next dependency**: `evidence_presence`, `entity_consistency`, and `period_consistency` are implemented and computed per change; `citation_support`, `numeric_consistency`, and `direction_consistency` are honestly recorded `not_run` and are the next work item together with step 8
8. Reachable review routing for comparison results (reusing the mandatory-review policy) — **the next dependency** alongside the remaining validators
