...
├── tests/           # 866 tests (including comparison regression, reliability, authenticated actors, and access control)
│   ├── test_ingest.py
│   ├── test_tools.py
│   ├── test_agent.py
│   ├── test_audit.py
...
866 tests are collected. In the credential-free run, **854 pass, 2 optional
...
full live run passes all 866 tests. The table below
...
**Time windows.** `since` / `until` are optional, timezone-aware, and inclusive (`since <= t <= until`); a naive timestamp is rejected rather than assumed to be UTC, and `until` must not precede `since`. Historical attempt metrics are windowed on `started_at`, historical replay metrics on `requested_at`. **Current-state gauges and the entire issue set are evaluated at query time and are never windowed** — "how many attempts are stuck right now" is not a question about a past interval, which is also why `unresolvedOperationalIssues` always equals the unfiltered issue total.
...
**Percentiles.** One definition everywhere: **nearest-rank**, stdlib only — sort ascending, `rank = ceil(p × n)`, take index `rank − 1`. For n = 1 every percentile is the sole value; for n = 2, p50 is the lower value and p95 the upper. Duration is `finished_at − started_at` over terminal attempts. A **negative** duration is excluded from the statistics, counted in `negativeDurationAttempts`, and raised as an issue — never folded into a min or mean that would then be quietly wrong.
...
**Fail-closed data validation.** Metrics come from stored rows, so contradictory rows would otherwise produce clean-looking numbers that are false. The report is **refused** with the stable code `reliability_data_invalid` when it finds a terminal attempt without `finished_at`, a running attempt with one, an attempt or comparison status outside the store's vocabulary, a replay whose source or replacement attempt is missing, or a stored timestamp that is not parseable as timezone-aware UTC. No partial metrics are returned after a structural error, on any surface. The one deliberate exception is a negative duration, which becomes an issue instead — a single skewed clock should not suppress the whole report. Detection transition *events* are deliberately not consumed: no metric in this contract needs them.

**Structured logging.** Lifecycle records cover comparison creation, detection
attempt start and terminal transitions, replay creation/completion, governance
evaluation, authenticated comparison-review decisions, and export creation.
...
authenticated mutations. They never include a bearer token, shared secret,
...
(Prometheus, OpenTelemetry, CloudWatch), and no scheduler, worker, queue, or
polling loop. Reliability API reads now require a verified local access token
...
* Reliability visibility is **read-only and exposes state only**: it never repairs, retries, replays, deletes, or mutates a workflow record. Its API routes require `reliability.read`; the local report CLI remains available to an operator with filesystem access. There is still no dashboard, frontend, alerting, notification, external monitoring integration, scheduler, worker, queue, or polling loop. Lifecycle fields travel through stdlib logging but are not guaranteed JSON. Replay eligibility still depends on the filing registry and fails closed when it cannot be evaluated. Local SQLite records and process logs remain application records, not tamper-proof storage
...
**Stage 3.5 work is in progress and not complete.** Four bounded steps have
...
What still does not exist is durable background execution: no automatic retry
or replay, timeout takeover, scheduler, queue, worker, lease, or heartbeat.
An interrupted detection stays `detecting`/`running` until an authorized human
acts, and a failed attempt is not retryable. Maturity therefore remains
**Stage 3**. Stage 3.5 remains in progress; the next major dependency is
**durable background execution with leases and heartbeats**.
...
12. Then Stage-3.5 reliability and authentication work (state machine, retries, durable failure handling, auth) — **in progress, not complete**. The first reliability step is done: comparison detection now has **durable execution attempts and an explicit lifecycle**. The comparison lifecycle is `ready_for_detection → detecting → detected | failed`, with `detecting` meaning a durable `running` attempt exists and no result has been committed for it. Every execution that starts gets an attempt row (stable id, monotonically increasing `attempt_number` per comparison, detector and workflow versions, the filing source hashes captured at start, `started_at`/`finished_at`, terminal result hash or safe failure code and bounded safe summary) plus append-only `detection_started` / `detection_succeeded` / `detection_failed` transition events. This holds for **every** runtime detection path — the API route, the CLI, and the comparison-regression runner all go through the same public entry point, and no production code persists a result without its attempt (the low-level `comparison_store.record_result` is a test-fixture primitive only, enforced by an audit test). A known post-start failure keeps its **own** stable domain code; only unexpected non-domain faults are recorded as `detector_internal_error`, and persisted summaries are always derived from the code, never from exception text. Both terminal transitions are **transactional**: start commits the attempt, the started event, and the `detecting` state in one transaction before any data is read, and exactly one of the completion or failure transactions finalizes it — so a committed result can never coexist with a running attempt, and a succeeded attempt can never exist without its result (enforced by storage CHECK constraints as well as by the transactions). Concurrent requests no longer duplicate detector work: a storage-level constraint admits at most one running attempt per comparison, and the loser gets `409 detection_in_progress` (or the winner's result if it already committed). Attempts and events are readable at `GET /api/comparisons/{id}/detection-attempts`, `GET /api/detection-attempts/{id}`, and `GET /api/detection-attempts/{id}/events`; the detect response additionally carries `attemptId` beside the unchanged `comparison.v1` result contract. **They are append-only application records, not tamper-proof storage.**
    ...
    The fourth bounded Stage-3.5 step is now done: **authenticated comparison-workflow actors and permission-based authorization**. A fail-fast checked-in policy defines local HS256 token verification, allowlisted roles, and exact permissions. Every comparison route declares its permission; authentication and authorization happen before resource lookup or mutation. New protected-API replay and review records derive actor identity from the immutable verified Principal and persist a narrow `local_hs256` source marker; direct library calls and historical rows remain `legacy_self_asserted`. Missing/invalid credentials use sanitized 401 responses and insufficient permissions use sanitized 403 responses. An offline local token CLI (no network or cloud credentials) supports controlled issuance from the required runtime signing secret. This is deliberately local shared-secret authentication, not OAuth/OIDC/SSO or account management. **Stage 3.5 remains in progress; the next major dependency is durable background execution with leases and heartbeats**
