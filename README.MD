...
* There is no filing comparison *analysis* yet: no section alignment, no change detector, and no comparison precision/recall evaluation. A persistent comparison entity now exists (`POST/GET /api/comparisons`, SQLite-backed): it validates and stores an eligible filing pair in an explicit `ready_for_detection` state — **persisted comparisons contain no detected changes**, and the pre-detection record is deliberately not a `comparison.v1` `ComparisonResult`, so nothing pretends a detector ran. The impact CLI still analyzes changed chunks of one document family (refusing families that span multiple filings) against past answers
...
4. Item 1A (Risk Factors) section alignment between the two filings — **the next step**
