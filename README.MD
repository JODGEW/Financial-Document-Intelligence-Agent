...
* Comparison detection now has a durable execution record and an explicit lifecycle (see below), but **there is still no retry, replay, dead-letter queue, timeout takeover, scheduler, or background worker anywhere**. Concretely: killing the process mid-detection leaves the comparison in `detecting` with a `running` attempt **indefinitely**, and every later detect request keeps returning `409 detection_in_progress`. Nothing infers termination from age, wall-clock, or file mtime; nothing marks that attempt failed on a guess; nothing starts a replacement. The state is made visible and queryable instead, so an operator can see it — bounded stale-attempt detection and operator-controlled replay are the next reliability commit. The generic chat side has no durable task record at all, and the audit log remains an append-only local file, not immutable storage
...
**Stage 3.5 reliability work has begun but is not complete.** The first step landed: comparison detection is now durably represented by execution attempts and append-only transition events, and the lifecycle explicitly includes `detecting` (roadmap step 12). What that does *not* yet include is the part that would make it self-healing — there is no retry, replay, timeout takeover, scheduler, or worker, so an interrupted detection stays `detecting`/`running` until an operator looks. Maturity therefore remains **Stage 3**, with the same qualifications above; the next dependency is bounded stale-attempt detection and operator-controlled replay.
