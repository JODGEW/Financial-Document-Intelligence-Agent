...
762 tests, all passing. **752 run credential-free**; exactly **10 need live Bedrock credentials plus an ingested corpus** — five in `test_agent.py` (`test_query_returns_output`, `test_query_uses_local_search`, `test_query_with_chat_history`, `test_output_contains_result_summary`, `test_unavailable_company_not_substituted`) and five in `test_tools.py` (`test_local_search_returns_results`, `test_local_search_includes_source_metadata`, `test_local_search_no_match`, `test_retriever_returns_documents`, `test_concurrent_local_search`). The counts and the credential split were measured by collecting the suite and re-running it with the credential chain removed, not estimated. The table below predates the comparison workflow and covers only the suites it lists. Run with:
...
**Structured logging.** Six lifecycle records are emitted at the existing mutation boundaries: `detection_attempt_started`, `detection_attempt_succeeded`, `detection_attempt_failed`, `detection_attempt_timed_out`, `detection_replay_created`, and `detection_replay_completed`. Fields (`event`, `comparison_id`, `attempt_id`, `attempt_number`, `replay_id`, `source_attempt_id`, `detector_version`, `workflow_version`, `status`, `failure_code`, `result_hash`, `elapsed_ms`) ride on the LogRecord through stdlib `logging`'s `extra`. **These are structured fields carried through stdlib logging — they are not guaranteed to be JSON**: no JSON formatter is configured anywhere in this repository, so what a handler renders is whatever that handler's formatter does. Records are emitted **after** the transaction they describe commits and are wrapped so that a logging fault can never abort a workflow transaction or roll back committed state; an idempotent replay that changed nothing logs nothing. Failure records carry the **stable failure code**, never exception text; unexpected faults keep their separate `logger.exception` correlated by comparison and attempt id, and the API's `error_id` behavior is unchanged. Evidence, document text, excerpts, reviewer notes, operator ids and notes, source paths, SQL, environment values, and credentials are absent by construction — the payload builder cannot emit a field that is not on the allowlist, and the store read behind the report never even selects the operator columns.

**What does not exist here.** No dashboard, no frontend surface, no alerting, no notifications (email, Slack, SNS), no external monitoring integration (Prometheus, OpenTelemetry, CloudWatch), no scheduler, worker, queue, or polling loop, and no authenticated operator or reviewer — anyone who can reach the local API or the local database can read all of it. Local SQLite records and process logs are **application records, not tamper-proof storage**: anyone with file access can alter them.
...
* Reliability visibility is **read-only and exposes state only**: it never repairs, retries, replays, deletes, or mutates a workflow record. Stale and failed work is *surfaced*, never acted on — an issue is calculated from source records at query time, there is no issue table and **no acknowledgement or resolution state**, so an issue keeps reappearing until the underlying record state changes (which only an operator's explicit replay, or accepting the failure, can do). Metrics derive entirely from local persisted records: nothing here has been measured against a real production workload, and the queries are unindexed scans sized for the tens-of-rows scale this prototype holds. There is **no dashboard, no frontend surface, no alerting, no notification (email/Slack/SNS), no external monitoring integration (Prometheus/OpenTelemetry/CloudWatch), no scheduler, worker, queue, or polling loop, and no authenticated operator** — anyone who can reach the local API or the local database file can read every reliability report. Lifecycle logs carry structured fields through stdlib `logging`, but **are not guaranteed to be JSON**: no JSON formatter is configured, so machine consumption depends on whatever formatter a deployment sets up. Replay eligibility is the one metric with an external dependency (the filing registry); when it cannot be evaluated the whole report is refused rather than reported as zero, which means a clean clone that has not run `python ingest.py` will legitimately get `reliability_dependency_unavailable` once a stale attempt exists. Local SQLite records and process logs remain application records, not tamper-proof storage
...
**Stage 3.5 reliability work is in progress and not complete.** Three steps have landed (roadmap step 12): comparison detection is durably represented by execution attempts and append-only transition events with an explicit `detecting` lifecycle state; an interrupted detection can be **recognized as stale under an explicit configured threshold and resolved by an explicit operator replay** — bounded by a maximum attempt count, atomic in one transaction, and durably attributed; and that persisted state is now **aggregable by an operator** through read-only reliability metrics, issue and failure listings, a credential-free CLI, and correlated structured lifecycle logs. What that still does *not* include is anything self-healing: there is no automatic retry, automatic replay, timeout takeover, scheduler, queue, or worker, so an interrupted detection stays `detecting`/`running` until a human acts on it, and a *failed* attempt is not retryable at all. The new visibility layer changes what an operator can *see*, not what the system does on its own — it exposes state and never repairs it, and there is still no dashboard, alerting, notification, external monitoring, or authenticated operator. Maturity therefore remains **Stage 3**, with the same qualifications above; the next dependency is **authenticated operator/reviewer identity and authorization** — every actor in this workflow (reviewer, replay operator, reliability reader) is currently self-asserted or anonymous, which bounds how far durable background execution is worth taking. If the roadmap instead prioritizes runtime recovery first, the alternative next step is durable background execution (a real work queue with at-least-once delivery and idempotent handlers) rather than the synchronous, operator-triggered execution used today.
...
12. Then Stage-3.5 reliability and authentication work (state machine, retries, durable failure handling, auth) — **in progress, not complete**. The first reliability step is done: comparison detection now has **durable execution attempts and an explicit lifecycle**. The comparison lifecycle is `ready_for_detection → detecting → detected | failed`, with `detecting` meaning a durable `running` attempt exists and no result has been committed for it. Every execution that starts gets an attempt row (stable id, monotonically increasing `attempt_number` per comparison, detector and workflow versions, the filing source hashes captured at start, `started_at`/`finished_at`, terminal result hash or safe failure code and bounded safe summary) plus append-only `detection_started` / `detection_succeeded` / `detection_failed` transition events. This holds for **every** runtime detection path — the API route, the CLI, and the comparison-regression runner all go through the same public entry point, and no production code persists a result without its attempt (the low-level `comparison_store.record_result` is a test-fixture primitive only, enforced by an audit test). A known post-start failure keeps its **own** stable domain code; only unexpected non-domain faults are recorded as `detector_internal_error`, and persisted summaries are always derived from the code, never from exception text. Both terminal transitions are **transactional**: start commits the attempt, the started event, and the `detecting` state in one transaction before any data is read, and exactly one of the completion or failure transactions finalizes it — so a committed result can never coexist with a running attempt, and a succeeded attempt can never exist without its result (enforced by storage CHECK constraints as well as by the transactions). Concurrent requests no longer duplicate detector work: a storage-level constraint admits at most one running attempt per comparison, and the loser gets `409 detection_in_progress` (or the winner's result if it already committed). Attempts and events are readable at `GET /api/comparisons/{id}/detection-attempts`, `GET /api/detection-attempts/{id}`, and `GET /api/detection-attempts/{id}/events`; the detect response additionally carries `attemptId` beside the unchanged `comparison.v1` result contract. **They are append-only application records, not tamper-proof storage.**
    ...
    The second reliability step is also done: **bounded stale-detection recognition and operator-controlled replay** ([detection_recovery.py](detection_recovery.py), policy in [policies/detection_recovery_policy.yaml](policies/detection_recovery_policy.yaml), fail-fast validated like the other governance YAML). An attempt is *stale* when `age >= stale_after_seconds` (default 900; the boundary is inclusive, naive timestamps are rejected, and a negative age from clock skew is never stale). **Staleness is eligibility, not an action:** crossing the threshold mutates nothing, and `GET /api/detection-attempts/{id}/recovery` is strictly read-only — it reports `isStale`, `replayEligible`, `attemptsUsed`/`maxAttempts`, and a `blockingReason` drawn from the same stable codes a replay would return, without touching state. An attempt becomes `timed_out` **only** inside an explicit `POST /api/detection-attempts/{id}/replay` transaction, which in one `BEGIN IMMEDIATE` re-reads the comparison and attempt, verifies the lifecycle and sole-running-attempt invariants, recomputes staleness from the injected clock, enforces `max_attempts_per_comparison` (default 3), revalidates the source hashes and detector/workflow versions, retires the stale attempt with a `detection_timed_out` event, starts the replacement running attempt with its `detection_started` event, and writes the durable replay linkage — leaving the comparison `detecting`. The replacement then executes synchronously through the same detector seam a first attempt uses. Replays carry a required self-asserted operator id (**not authenticated**), an allowlisted reason code, and a required bounded note; they are idempotent by canonical request hash (a byte-equivalent repeat returns the stored replay and the replacement's current outcome without re-executing), one stale attempt yields at most one replacement (`409` for a different request), and changed inputs or versions refuse with `detection_replay_inputs_changed` / `detection_replay_version_changed` rather than silently comparing different content. **There is still no automatic retry, automatic replay, timeout takeover, scheduler, queue, or worker** — a process kill leaves `detecting`/`running` until a human acts, and a failed attempt is not retryable

    The third reliability step is also done: **read-only operator visibility, structured reliability metrics, and correlated transition logging** ([comparison_reliability.py](comparison_reliability.py), CLI in [scripts/comparison_reliability_report.py](scripts/comparison_reliability_report.py), full contract in the [Reliability visibility and operations](#reliability-visibility-and-operations) section above). Before it, an operator could inspect one attempt or replay *if they already knew the id*; the aggregate questions — how many comparisons are detecting, how many running attempts are stale, which need attention, how attempts and replays actually turned out, which failure codes dominate, which detector/workflow versions produced the records — required querying SQLite by hand. Now they are answerable through `GET /api/comparison-reliability/summary` / `.../issues` / `.../failures` and a credential-free local CLI. **This step exposes state and nothing else**: the only SQLite access is a `mode=ro` snapshot read (a write is refused by the driver), replay eligibility comes from a pure record-based recovery calculation shared with the recovery endpoint, no reliability path can initialize or migrate SQLite even transitively, and stale or failed work is surfaced but never retried, replayed, repaired, or deleted. Rates carry explicit numerators and denominators (attempt rates over `terminal_attempts`, replay success over terminal replay replacements, running work never in a denominator), zero denominators are `null` rather than NaN or a fake 0.0, percentiles are nearest-rank (`ceil(p × n)`), and windows are inclusive timezone-aware UTC on `started_at`/`requested_at` while current-state gauges and issues stay evaluated at query time. Issues are calculated from source records rather than stored — no issue table, **no acknowledgement state** — with a closed field allowlist and stable `recommendedActionCode` values instead of prose. Contradictory stored rows **fail the report closed** (`reliability_data_invalid`) instead of yielding clean-looking numbers, with a negative duration the one deliberate exception (excluded from the statistics and raised as an issue). Unobservable storage fails closed the same way: an initialized database with no records is a valid empty system and reports zeros, but a missing or unreadable database (`reliability_storage_unavailable`) or one missing a required workflow table (`reliability_data_invalid`) is refused rather than reported as empty, and the read path never creates or migrates storage to make the question answerable. Six lifecycle records — `detection_attempt_started` / `_succeeded` / `_failed` / `_timed_out`, `detection_replay_created`, `detection_replay_completed` — are emitted after their transaction commits with an allowlisted field set carried through stdlib `logging`; **they are structured fields, not guaranteed JSON**, they never carry operator prose or exception text, and a logging fault cannot roll back committed workflow state. No mutation route, retry, scheduler, worker, queue, notification, dashboard, external monitoring integration, or authentication was added. **The next dependency is authenticated operator/reviewer identity and authorization** (or durable background execution, if runtime recovery is prioritized first), then the remaining Stage-3.5 work alongside real public-filing evaluation
...
* [~] Logging and monitoring — the comparison workflow has read-only reliability metrics, issue/failure listings, a local CLI ([scripts/comparison_reliability_report.py](scripts/comparison_reliability_report.py)), and correlated structured lifecycle logs carried through stdlib `logging`. **That is local visibility, not monitoring**: no dashboard, no alerting, no notifications, no external monitoring backend (Prometheus/OpenTelemetry/CloudWatch), no guaranteed-JSON log format, no log shipping, and no authenticated operator. The generic chat side still has no durable task record to report on at all
